...
public class GameOfLife : ProcessingLite.GP21
{
	...
	void Start()
	{
		...
		cellsBuffer = new GameCell[numberOfColums, numberOfRows];
		//Create all objects
		...
		for (int y = 0; y < numberOfRows; y++)
		{
			...
			for (int x = 0; x < numberOfColums; x++)
			{
				...
				cellsBuffer[x, y] = new GameCell(x * cellSize, y * cellSize, cellSize);
		...
		for (int x = 0; x < numberOfColums; x++)
		{
			for (int y = 0; y < numberOfRows; y++)
			{
				...
				for (int xx = x - 1; xx <= x + 1; xx++)
				{
					...
					{
						if (((xx > 0) && (xx < numberOfColums)) && ((yy > 0) && (yy < numberOfRows)))
						{
							if (!((xx == x) && (yy == y)))
							{
